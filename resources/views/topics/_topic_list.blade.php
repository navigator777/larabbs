			...
			<div class="media-body">
				...
				<div class="media-body meta">
					<a href="" title="{{ $topic->category->name }}">
						<span class="glyphocon-folder-open" aria-hidden="true"></span>
						{{ $topic->category->name }}
					</a>

					<span> · </span>
					<a href="{{ route('users.show',[$topic->user_id]) }}" title="{{ $topic->user_id }}">
						<span class="glyphicon glyphicon-user" aria-hidden="true"></span>
						{{ $topic->user->name }}
					</a>
					<span> · </span>
					<span class="glyphicon glyphicon-time" aria-hidden="true"></span>
					<span class="timeago" title="最后活跃与">{{ $topic->updated_at->diffForHumans() }}</span>
				</div>
