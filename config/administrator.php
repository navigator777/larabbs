...
return array(
     ...
     *数组里的值有三种类型:
     *1.字符串 - 子菜单的入口,不可访问;
     *2.模型配置文件 - 访问 model_config_path 目录下的模型文件,如 users 访问的是 uses.php 模型配置文件;
     *3.配置信息 - 必须使用前缀 settings. ,对应 settings_config_path 目录下的文件,如:默认设置下,
     *                  settings.site 访问的是 administrator/settings/site.php 文件
     *页面文件 - 必须使用前缀 page. 如 page.pages.analytics 对应 administrator/pages/analytics.php 
     ...
     *权限控制的回调函数.
     *
     *此回调函数需要返回true 或 false,用来检测当前用户是否有权限访问后台
     * true为通过,false会将页面重定向到 login_path 选项定义的URL中
     * @type closure
     */
    'permission' => function () {
     ...
     *设置后台主页视图文件,有 use_dashboard 选项决定
     * @type string
     */
    'dashboard_view' => '',
     ...
     *用来作为后台主页的菜单条目,有 use_dashboard 选项决定,菜单指的是 menu选项 
     * @type string
     */
    'home_page' => 'users',
